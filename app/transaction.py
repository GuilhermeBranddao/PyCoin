...
class Transaction:
    ...
    def load_private_key_from_string(self, key_string):
        """
        Carrega uma chave privada de uma string simplificada (sem marcações BEGIN/END) para o formato PEM e a converte em um objeto.
        ...
        try:
            pem_formatted_key = (
                "-----BEGIN PRIVATE KEY-----\n"
                + '\n'.join(key_string[i:i+64] for i in range(0, len(key_string), 64))  # Quebra em linhas de 64 caracteres
                + "\n-----END PRIVATE KEY-----"
            )

            private_key = serialization.load_pem_private_key(pem_formatted_key.encode('utf-8'), password=None)
            return private_key
        except Exception as e:
            print(f"Erro ao carregar chave privada: {e}")
            return None

    def load_public_key_from_string(self, key_string):
        """
        Carrega uma chave pública de uma string simplificada (sem marcações BEGIN/END) para o formato PEM e a converte em um objeto.
        ...
        try:
            ...
            public_key = serialization.load_pem_public_key(pem_formatted_key.encode('utf-8'))
            return public_key
        except Exception as e:
    ...
    def verify_signature(self):
        ...
        try:
            self.public_key.verify(self.signature, transaction_data, ec.ECDSA(hashes.SHA256()))
            return True
        except InvalidSignature:
            return False
    ...
    def _get_transaction_data(self):
        """
        Concatena os dados da transação (sender, recipient, amount) em bytes com delimitadores.
        :return: Dados da transação em formato binário.
        """
        transaction_data = f"{self.sender}|{self.recipient_address}|{self.amount}"
        return transaction_data.encode('utf-8')

    def _decode_transaction_data(self, transaction_data):
        """
