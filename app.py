...
def get_chain():
    response = {'chain': blockchain.blockchain, 'length': len(blockchain.blockchain)}
    return jsonify(response), 200
...
def add_transaction():
    ...
    index = blockchain.add_transaction(
        private_key_sender=json['private_key_sender'],
...
def new_blockchain():
    ...
    print('INFO>>>>>', json['nodes_updated'])
    response = blockchain.check_progagate_blockchain(
        new_blockchain=json['chain'], nodes_updated=json['nodes_updated']
    )
